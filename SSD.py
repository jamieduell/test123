...
dementia = DATASET()
#df = dataframe
df = pd.DataFrame(np.c_[dementia['data'], dementia['target']],
columns= np.append(dementia['feature_names'], ['target']))
...
df.replace(to_replace={'target': {0: dementia.target_names[0]}}, inplace=True)
df.replace(to_replace={'target': {1: dementia.target_names[1]}}, inplace=True)
...
#print("KNeighbors Accuracy: {0:.3%}".format(accuracy_score(prediction, y_test)))
#print("Cross validation score: {0:.3%} (+/- {1:.3%})".format(np.mean(k_scores), np.std(k_scores)*2))
#print("Error: {0:.3%}".format(1-(accuracy_score(prediction, y_test))))
#print("Execution time: %s seconds \n" % "{0:.5}".format(end-start))

#end = time.time()
#most work carried out developed through the guidance of the sklearn documentation with various implementations
#from varying sources to mold the project with correct code implementation

#confusion mtrx

#confusion_matrix(y_test, prediction, labels=df['target'])
#plt.show()
...
#detector.loadModel()
#detections, extracted_images = detector.detectObjectsFromImage(input_image=os.path.join(execution_path , "malignant.jpg"), output_image_path=os.path.join(execution_path , "imagenew.jpg"), extract_detected_objects=True)
